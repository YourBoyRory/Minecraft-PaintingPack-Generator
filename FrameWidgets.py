from PyQt5.QtCore import Qt, QUrl, QSize, QTimer, QStringListModel, pyqtSignal, QByteArray
...
class MusicEditor(QWidget):
        ...
        if mime_data.hasUrls():
            ...
            for url in urls:
                ...
                if any(file_path.lower().endswith(ext) for ext in image_extensions):
                    ...
                    mp3Data.export(self.audioData, format="ogg")
                    self.parent.setButtonEnabled(True)
                elif file_path.lower().endswith('.ogg'):
                    print(f"Native File Dropped: {file_path}")
                    with open(file_path, 'rb') as f:
                        file_data = f.read()
                        self.audioData = BytesIO(file_data)
                    self.parent.setButtonEnabled(True)
                else:
                    print(f"Unsupported File: {file_path}")

    def reset(self):
        self.disc_combo_box.clear()
        self.disc_combo_box.addItems(self.discs.keys())
        self.updateTextureComboBox()
    ...
    def requestViewPortDraw(self):
        if self.texture_combo_box.currentText() == 'Custom':
            ...
            if self.customDiscArtPath:
                try:
                    self.discArtPath = self.customDiscArtPath
                    pixmap = QPixmap(self.customDiscArtPath)
                    scale = self.detail_spin_box.value()*16
                ...
                self.discArt.setText(" ")
                return
        else:
            self.loadedFile.setText("Drop Audio File Here\n\n")
            self.setTextureVisible(False)
            filename = self.discs[self.texture_combo_box.currentText()]['file_name']
            self.discArtPath = f"./assets/disc/{filename}.png"
...
class PackControls(QWidget):
    ...
    def __init__(self, parent):
        ...
        self.add_button.clicked.connect(self.writeFile)
    ...
    def writeFile(self):
        if self.parent.viewport.currentIndex() == 1:
            self.writeAudio()
        else:
            self.writeImage()
